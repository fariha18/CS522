...
public class ChatHelper {
    ...
    public void postMessage(String chatRoom, String messageText) {
        if (messageText != null && !messageText.isEmpty()) {
            ...
            msg.appID = Settings.getAppId(context);
            msg.chatroom = chatRoom;
            msg.timestamp = DateUtils.now();
            msg.latitude = location.getLatitude();
            msg.longitude = location.getLongitude();
            msg.sender = Settings.getChatName(context);

            Bundle data = new Bundle();
            data.putParcelable(PostMessageWorker.MESSAGE_KEY, msg);
    ...
    public void startMessageSync() {
        if (Settings.SYNC) {
            ...
            Bundle data = null;
            syncRequest= new PeriodicWorkRequest(PostMessageWorker.class, null,SYNC_INTERVAL);
