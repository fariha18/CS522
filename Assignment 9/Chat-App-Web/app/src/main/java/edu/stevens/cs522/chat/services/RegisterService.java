    ...
    public int onStartCommand(@NonNull Intent intent, int flags, final int startId) {
        ...
        executor.execute(() -> {
            ...
            try {
                ...
                Log.d(TAG, "Registration service threw an exception....", e);
                ...
                mainLoop.post(() -> {
                    ...
                    if (registerResponse != null && !(registerResponse instanceof ErrorResponse)) {
                        // TODO let user know request succeeded (with another notification)
                        Toast.makeText(context,"request successful",Toast.LENGTH_SHORT).show();
                    } else {
                        // TODO let user know request failed (with another notification)
